...
function createItem(): BudgetItem {
return {
...
const [items, setItems] = useState<BudgetItem[]>([createItem()]);
...
items.reduce(
(sum, item) =>
sum + Number(item.quantity || 0) * Number(item.unitValue || 0),
0
),
[items]
);
...
100,
Math.max(0, numberValue(discountPercent))
);
...
const desiredNumber = numberValue(desiredAmount);
...
const formattedBudgets: Budget[] =
  (budgetsResult.data ?? []).map((item) => {
    ...
    if (Array.isArray(item.itens) && item.itens.length > 0) {
      loadedItems = item.itens.map((entry: Partial<BudgetItem>, index: number) => {
        ...
        return {
          ...
          unitValue,
          total: Number(
            entry.total ?? quantity * unitValue
          ),
        };
      });
    }
    ...
    if (loadedItems.length === 0) {
      loadedItems = [
        {
          id: `legacy-${item.id}`,
          description: item.servico ?? "Serviço",
          quantity: 1,
    ...
    const loadedSubtotal =
      ...
      loadedItems.reduce((sum, entry) => sum + entry.total, 0);
    ...
    const loadedDiscountValue =
      ...
      loadedSubtotal * (loadedDiscountPercent / 100);
    ...
    const loadedFinal =
      ...
      Math.max(0, legacyValue || loadedReference - loadedDiscountValue);
    ...
    return {
      ...
      service:
        item.servico ??
        loadedItems.map((entry) => entry.description).join(", "),
      ...
      discountPercent: loadedDiscountPercent,
      discountValue: loadedDiscountValue,
...
if (item.id !== id) return item;
    ...
    if (field === "quantity") {
      ...
      return {
        ...item,
        quantity,
        total: quantity * item.unitValue,
      };
    }

    if (field === "unitValue") {
      const unitValue = Math.max(0, numberValue(value));
...
if (desiredNumber <= 0) {
setNegotiationMessage(
"Informe primeiro quanto você quer receber."
);
return;
}
...
if (discountNumber <= 0) {
  setNegotiationMessage(
    "Informe uma porcentagem de desconto maior que zero."
  );
  return;
}
...
if (calculatedReference <= desiredNumber) {
  setNegotiationMessage(
    "O valor de referência precisa ser maior que o valor final desejado."
  );
  return;
}
...
if (subtotal <= 0) {
  setNegotiationMessage(
    "Adicione pelo menos um serviço com valor antes de aplicar a negociação."
  );
  return;
}

const factor = calculatedReference / subtotal;

setItems((current) =>
  current.map((item) => {
    const newUnitValue = item.unitValue * factor;

    return {
      ...item,
      unitValue: newUnitValue,
      total: item.quantity * newUnitValue,
    };
  })
);

setNegotiationMessage(
  `Negociação aplicada. Valor de referência: ${money(
    calculatedReference
  )}. Total desejado: ${money(desiredNumber)}.`
);
...
if (normalized.includes("instala")) return "Instalação";
if (normalized.includes("higien")) return "Higienização";
if (normalized.includes("corret")) return "Corretiva";
if (normalized.includes("prevent")) return "Preventiva";
...
const confirmed = window.confirm(
  `Gerar Ordem de Serviço para ${budget.client}?\n\n` +
    ...
    `Valor final: ${money(budget.finalValue)}`
);
...
try {
  ...
  const alreadyGenerated = (existingOrders ?? []).some(
    (order) =>
  ...
  const serviceDescription = budget.items
  ...
  const serviceType = getServiceType(budget.service);
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      descricao: serviceDescription || budget.service,
...
try {
  ...
  const calculatedDiscount =
    calculatedSubtotal * (discountNumber / 100);
  ...
  const serviceDescription = normalizedItems
  ...
  const { error } = await supabase
    ...
    .insert({
      ...
      equipamentos: equipment.trim() || null,
      valor: calculatedFinal,
      data: date || null,
      status: "Rascunho",
      ...
      desconto_valor: calculatedDiscount,
      valor_referencia: calculatedSubtotal,
      valor_final: calculatedFinal,
    });

  if (error) throw error;

  alert(`Orçamento ${number} salvo com sucesso!`);
...
function openPreview(budget: Budget) {
...
.map(
(item) => "<tr> <td>${item.description}</td> <td>${item.quantity}</td> <td>${money(item.unitValue)}</td> <td>${money(item.total)}</td> </tr>"
)
.join("");
  ...
  <body>
    <div class="header">
      ...
      <div>
        ...
        ${budget.number}<br>
        ${budget.date || new Date().toLocaleDateString("pt-BR")}
      </div>
    </div>

    <div class="client">
      <strong>Cliente:</strong> ${budget.client}<br>
      <strong>Cidade:</strong> ${budget.city}
    </div>
    ...
    <div class="totals">
      <div class="row">
        <span>Subtotal</span>
        <strong>${money(budget.referenceValue)}</strong>
      </div>
      ...
      ${
        budget.discountPercent > 0
          ? `
            <div class="row">
              <span>Desconto especial (${budget.discountPercent.toFixed(
                2
              )}%)</span>
              <strong>- ${money(budget.discountValue)}</strong>
            </div>
          `
          : ""
      }
      ...
      <div class="row final">
        <span>Total</span>
        <strong>${money(budget.finalValue)}</strong>
      </div>
    </div>
...
function sendWhatsApp(budget: Budget) {
const lines = budget.items
.map(
(item) =>
"• ${item.quantity}x ${item.description} — ${money( item.total )}"
)
.join("\n");
...
const message =
  ...
  `*Orçamento ${budget.number}*\n\n` +
  `${lines}\n\n` +
  `Subtotal: ${money(budget.referenceValue)}\n` +
  `${
    budget.discountPercent > 0
      ? `Desconto especial: ${budget.discountPercent.toFixed(
          2
        )}% (-${money(budget.discountValue)})\n`
      : ""
  }` +
  `*Total: ${money(budget.finalValue)}*\n\n` +
  `Qualidade e confiança em todos os detalhes.`;
...
(sum, budget) => sum + budget.finalValue,
0
);
...
Controle de propostas, serviços e negociações.
</p>
</div>
    ...
    <div className="mb-5 flex flex-col gap-3 md:flex-row">
      ...
      <select
        ...
        onChange={(event) =>
          setStatusFilter(
            event.target.value as "Todos" | BudgetStatus
          )
        }
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  {budget.discountPercent > 0 && (
                    <span className="rounded-full bg-amber-500/10 px-2.5 py-1 text-xs font-semibold text-amber-400">
                      {budget.discountPercent.toFixed(2)}% desconto
                    </span>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-3 lg:items-end">
                <div className="text-xl font-bold">
                  {money(budget.finalValue)}
                </div>

                {budget.discountPercent > 0 && (
                  <div className="text-xs text-slate-500">
                    Referência: {money(budget.referenceValue)}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {budget.status === "Aprovado" && (
                    <button
                      onClick={() =>
                        generateServiceOrder(budget)
                      }
                      disabled={
                        generatingOrderId === budget.id
                      }
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      changeStatus(
                        budget.id,
                        event.target.value as BudgetStatus
                      )
                    }
              ...
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Você pode colocar vários serviços no mesmo orçamento.
                  </p>
                </div>
                  ...
                  >
                    ...
                    <div className="grid gap-3 md:grid-cols-12">
                      ...
                      <div className="md:col-span-2">
                        ...
                        <div className="rounded-lg border border-slate-800 bg-slate-950 px-3 py-3 text-sm font-bold">
                          {money(
                            item.quantity *
                              item.unitValue
                          )}
                ...
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-200/60">
                      Esta área é somente para você. Não aparece para o cliente.
                    </p>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="rounded-xl bg-slate-950 p-4">
                      ...
                      <p className="mt-1 text-2xl font-bold text-amber-300">
                        {money(calculatedReference)}
                      </p>

                      <p className="mt-2 text-xs text-slate-500">
                        O cliente verá esse valor como subtotal e verá o desconto. Seu valor desejado ficará somente nesta área.
                      </p>
                    </div>
    ...
    <div className="fixed inset-0 z-[60] overflow-y-auto bg-black/80 p-3 md:p-6">
      <div className="mx-auto my-4 max-w-3xl rounded-2xl bg-white text-slate-900 shadow-2xl md:my-10">
        <div className="flex items-center justify-between border-b border-slate-200 p-5">
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold">
              Orçamento {previewBudget.number}
            </h2>
          </div>
        ...
        <div className="space-y-6 p-5 md:p-8">
          ...
          <div className="ml-auto max-w-sm space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <strong>
                {money(previewBudget.referenceValue)}
              </strong>
            </div>
            ...
            {previewBudget.discountPercent > 0 && (
              <div className="flex justify-between text-sm">
                ...
                <strong className="text-red-600">
                  - {money(previewBudget.discountValue)}
                </strong>
              </div>
            )}
            ...
            <div className="flex justify-between border-t border-slate-300 pt-3 text-xl">
              ...
              <strong className="text-emerald-600">
                {money(previewBudget.finalValue)}
              </strong>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 p-5 sm:flex-row sm:justify-end">
          ...
          <button
            onClick={() => printBudget(previewBudget)}
          ...
          <button
            onClick={() => sendWhatsApp(previewBudget)}
